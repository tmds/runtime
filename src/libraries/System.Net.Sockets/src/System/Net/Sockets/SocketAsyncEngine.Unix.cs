...
#pragma warning restore CA1802
        ...
        //
        // Queue of socket operations generated by EventLoop() that would be processed by the thread pool
        //
        private readonly ConcurrentQueue<object> _operationQueue = new ConcurrentQueue<object>();
        ...
        private void EventLoop()
        {
            try
            {
                ...
                ConcurrentQueue<object> operationQueue = _operationQueue;
                while (!shutdown)
                {
                    ...
                    for (int i = 0; i < numEvents; i++)
                    {
                        ...
                        else
                        {
                            ...
                            if (context != null)
                            {
                                context.HandleEvents(_buffer[i].Events, operationQueue);
        ...
        void IThreadPoolWorkItem.Execute()
        {
            ...
            if (!operationQueue.TryDequeue(out object? operation))
            {
                return;
            }
            ...
            while (true)
            {
                Debug.Assert(operation is IThreadPoolWorkItem);
                Unsafe.As<IThreadPoolWorkItem>(operation).Execute();
